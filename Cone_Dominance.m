...
train_rgby = wavelength_rgb(680)
test_rgby = wavelength_rgb(580)

[th_ctl,od_ctl]=ocular_dominance(train_rgby(1), train_rgby(2), test_rgby(1), test_rgby(2)); % run model with both eyes stimulated
od_ctl = od_ctl / max(abs(od_ctl)); % normalize the ocular dominance metric between -1 and 1

[th_dep,od_dep]=ocular_dominance(0, 0, test_rgby(1), test_rgby(2)); % run model with only right eye stimulated
...
function[v1_thr, OD] = ocular_dominance(leye_stim, reye_stim, ltest, rtest)
...
i_eye(1:num_eye/2) = i_eye(1:num_eye/2)+ltest;
for n = 1:num_eye
    eye(n) = sigmoid(i_eye(n),gain);
end
i_v1 = zeros(1,num_v1);
...
i_eye = spont_act*ones(1,num_eye);
i_eye(num_eye/2+1:num_eye) = i_eye(num_eye/2+1:num_eye)+rtest;
for n = 1:num_eye
    eye(n) = sigmoid(i_eye(n),gain);
end
i_v1 = zeros(1,num_v1);
